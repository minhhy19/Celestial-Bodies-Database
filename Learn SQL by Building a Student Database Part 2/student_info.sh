...
echo "$($PSQL "SELECT course FROM students INNER JOIN majors USING(major_id) LEFT JOIN majors_courses USING(major_id) LEFT JOIN courses USING(course_id) GROUP BY course HAVING COUNT(*) = 1 ORDER BY course")"