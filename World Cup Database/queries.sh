...
echo "$($PSQL "SELECT name FROM games AS g INNER JOIN teams AS t1 ON g.winner_id = t1.team_id OR g.opponent_id = t1.team_id WHERE year = 2014 AND round = 'Eighth-Final' GROUP BY name")"
...
echo "$($PSQL "SELECT name FROM games INNER JOIN teams ON games.winner_id = teams.team_id OR games.opponent_id = teams.team_id GROUP BY name HAVING name LIKE 'Co%'")"
